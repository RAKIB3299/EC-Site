...
/**
 * HTTPリクエストを受け取り、ProductImageFileServlet に対応する画面処理を行うサーブレットです。
 *
...
protected void doGet(HttpServletRequest request,HttpServletResponse response)throws IOException{
        ...
        if(path==null||path.length()<2||path.substring(1).contains("/")||path.contains("\\")||path.contains("..")){response.sendError(404);return;}
        Path file=ProductImageStorage.resolveForRead(path.substring(1));
        ...
        response.setContentLengthLong(Files.size(file));response.setHeader("Cache-Control","public, max-age=86400");
